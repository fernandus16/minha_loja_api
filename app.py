...
@app.get('/produtos_por_nome', tags=[produto_tag],
         responses={"200": ListagemProdutosSchema, "404": ErrorSchema})
def get_produtos_por_nome(query: ProdutoBuscaSchema):
    """Faz a busca por todos os Produto cadastrados
    ...
    session = Session()
    # fazendo a busca
    produtos = session.query(Produto).filter(Produto.nome == nome_produto).all()
